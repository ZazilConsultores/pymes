...
class My_Pdf_Page extends Zend_Pdf_Page {
	 ...
	 * @param bool $inContentArea
	 */
	public function drawText($text, $x1, $y1, $charEncoding = "UTF-8", $inContentArea=true){
		//move origin
		if($inContentArea){
			$y1 = $this->getHeight() - $y1 - $this->getMargin(My_Pdf::TOP);
			$x1 = $x1  + $this->getMargin(My_Pdf::LEFT);
	...
	/**
	 * $image es de tipo Zend_Pdf_Resource_Image, lanza fatal error si no se declara asi
	 */
