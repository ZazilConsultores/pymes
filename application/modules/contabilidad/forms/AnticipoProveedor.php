...
class Contabilidad_Form_AnticipoProveedor extends Zend_Form
{
    ...
    public function init()
    {
		...
		foreach ($tipoMovimientoDAO->obtenerTiposMovimientos() as $fila) {
			if ($fila->getIdTipoMovimiento() == "18") {
		...
		$eProv = new Zend_Form_Element_Select('idCoP');
		$eProv->setLabel('Seleccionar Proveedor:');
		$eProv->setAttrib("class", "form-control");
		
		foreach ($rowset as $fila) {
			$eProv->addMultiOption($fila->idProveedores, $fila->razonSocial);
		}
